    ...
    <header class="headerbar">
      <div class="header-inner container-xxl">
        ...
        <div class="left d-flex align-items-center gap-2">
          <div class="search-box">
            <input
              ...
              :disabled="loading"
              aria-label="Search accepted users by company or email"
            />
          </div>
        </div>
        ...
        <div class="right d-flex align-items-center gap-3">
          <h6 class="title mb-0">Accepted Accounts</h6>
            ...
            <div class="card user-card h-100">
              <div class="card-body d-flex flex-column">
                ...
                <div class="mt-auto d-flex flex-wrap gap-2">
                  ...
                  >
                    <span v-if="processing.id === user._id && processing.type === 'BLOCK'"
                          class="spinner-border spinner-border-sm me-1" role="status" aria-hidden="true"></span>
                    Block
                  </button>

                  <a
                    :href="`${config.public.AUTH}${user.domain}.${config.public.DOMAIN}`"
                    class="btn btn-outline-dark btn-sm flex-fill text-decoration-none"
                  ...
                  <button
                    ...
                    @click="createDatabaseFor(user._id)"
                    :disabled="isProcessing(user._id)"
                  >
                    <span v-if="processing.id === user._id && processing.type === 'DB'"
                          class="spinner-border spinner-border-sm me-1" role="status" aria-hidden="true"></span>
                    Create Database
                  </button>
                </div>
...
const search = ref('');
const processing = reactive({ id: null, type: null }); // 'BLOCK' | 'DB' | null
...
const blockUser = async (_id) => {
  processing.id = _id; processing.type = 'BLOCK';
  try {
    const projection = { status: USER_STATUS.BLOCK || 'BLOCK' };
...
const createDatabaseFor = async (_id) => {
  processing.id = _id; processing.type = 'DB';
  try {
