<template>
  <div class="layout-fab" :class="{ open: isOpen }" role="region" aria-label="Layout control">
    <div class="fab-shell" ref="shell">
      <button class="fab-toggle" type="button" :aria-expanded="isOpen" :aria-controls="ids.panel" @click="toggle"
        title="Header layout">
      ...
      <div class="fab-content" :id="ids.panel">
        <select id="position" name="position" class="mini-select" v-model="layout.position" @change="save">
...
.icon {
  ...
  stroke-width: 1.5;
  fill: currentColor;
  /* header bar uses fill; content rect has opacity */
}
